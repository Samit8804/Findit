...
function getExpiryDisplay(expiresAt: string | null | undefined, status: DisplayStatus): { text: string; icon: React.ReactNode; color: string } {
  if (!expiresAt) {
    return { text: 'No expiry', icon: <Clock className="w-3.5 h-3.5" />, color: 'text-slate-400' };
  }
  ...
  if (status === 'Expired' || expiryDate <= now) {
    return {
      text: `Expired on ${expiryDate.toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}`,
  ...
  if (diffDays <= 1) {
    return {
      text: diffDays <= 0 ? 'Expires today' : 'Expires tomorrow',
            ...
            <table className="w-full text-sm min-w-[1000px]">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/60 text-left text-[11px] uppercase tracking-wider text-slate-400">
                  ...
                  <th className="px-3 py-3.5 font-bold">Expiry</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {visible.map((ad) => {
                  const expiryInfo = getExpiryDisplay(ad.expiresAt, ad.display);
                  return (
                    <tr key={ad.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-3 py-3.5">
                        <span className={`inline-flex items-center gap-1.5 text-xs font-medium ${expiryInfo.color}`}>
                          {expiryInfo.icon}
                          {expiryInfo.text}
                        </span>
                      </td>
                  ...
                  <div className="flex gap-3 p-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-1.5">
                        <span className={`inline-flex items-center gap-1 text-[10px] font-medium ${expiryInfo.color}`}>
                          {expiryInfo.icon}
                          {expiryInfo.text}
                        </span>
                      </div>
