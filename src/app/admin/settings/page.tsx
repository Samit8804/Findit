  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-xs text-black mt-1">Global platform configuration.</p>
      </div>

      {GROUPS.map((g) => (
        <div key={g.title} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-5 py-3.5 border-b border-slate-100 bg-slate-50/60">
            <h2 className="text-xs font-black uppercase tracking-widest text-black">{g.title}</h2>
          </div>
            ...
            {g.items.map((item) => (
              <label key={item.key} className="flex items-center justify-between gap-4 p-5 cursor-pointer">
                <span>
                  ...
                  <span className="block text-xs text-black mt-0.5">{item.desc}</span>
                </span>
                <button
                  type="button"
                  role="switch"
      ...
      <div className="flex items-center justify-end gap-4">
        ...
        <Link href="/" className="text-xs font-semibold text-black hover:text-[#E53935]">Back to public site</Link>
        <button
          onClick={() => {
            setSaved(true);
