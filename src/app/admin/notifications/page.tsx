...
const META: Record<NotificationType | 'default', { icon: React.ElementType; classes: string }> = {
  ...
  default: { icon: Bell, classes: 'bg-slate-100 text-black' },
};
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-black mt-1">Platform-wide system and moderation alerts.</p>
      </div>
            ...
            >
              ...
              <span className="min-w-0 flex-grow">
                <span className="flex items-center justify-between gap-3">
                  <span className={`text-sm ${n.unread ? 'font-bold' : 'font-semibold text-black'}`}>{n.title}</span>
                  <span className="text-[10px] text-black shrink-0">{n.time}</span>
                </span>
                <span className="block text-xs text-black mt-1 leading-relaxed">{n.body}</span>
              </span>
