    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-black mt-1">Verified revenue and transactions across all promotions.</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        {cards.map((c) => (
          <div key={c.label} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
            ...
            <p className="text-[10px] font-semibold text-black mt-0.5 leading-tight">{c.label}</p>
          </div>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="p-10 text-center bg-white rounded-2xl border border-slate-100">
          <p className="text-sm font-semibold text-black">Connect Supabase keys to view real payment data.</p>
        </div>
      ) : visible.length === 0 ? (
        <div className="p-12 text-center bg-white rounded-2xl border border-slate-100 shadow-sm">
          <p className="text-sm font-semibold text-black">No payments in this status.</p>
        </div>
      ) : (
        <DataTable headers={['Order ID', 'User', 'Advertisement', 'Product', 'Amount', 'Provider', 'Status', 'Date']}>
          {visible.map((o) => (
            <tr key={o.id} className="hover:bg-slate-50/60 transition-colors">
              ...
              <td className="px-3 py-3.5 text-black whitespace-nowrap max-w-[160px] truncate">{o.user_email ?? '—'}</td>
              <td className="px-3 py-3.5 text-black max-w-[200px] truncate">{o.adTitle}</td>
              <td className="px-3 py-3.5 text-black whitespace-nowrap">{o.productName}</td>
              <td className="px-3 py-3.5 font-bold whitespace-nowrap">₹{o.amount.toLocaleString('en-IN')}</td>
              <td className="px-3 py-3.5 text-black whitespace-nowrap capitalize">{o.provider}</td>
              <td className="px-3 py-3.5">
                <span className={`inline-flex px-2.5 py-1 rounded-full text-[11px] font-bold ${badge(o.status)}`}>{o.status}</span>
              </td>
              <td className="pr-5 pl-3 py-3.5 text-black whitespace-nowrap">{o.date}</td>
            </tr>
          ))}
        </DataTable>
      )}
