    ...
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-red-100 p-10 text-center shadow-sm">
          ...
          <p className="text-xs text-slate-500 mt-1">Only moderators can read/update reports (RLS: public.is_moderator()).</p>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-14 text-center shadow-sm">
          ...
          <p className="text-xs text-slate-400 mt-1">Try a different status, priority or search.</p>
          <button
            onClick={() => {
              setSearch('');
              setStatusFilter('All');
        ...
        <>
              ...
              return (
                <tr key={r.id} className="hover:bg-slate-50/60 transition-colors">
                  <td className="pl-5 pr-3 py-3.5">
                    ...
                    <span className="block text-[11px] text-slate-400 font-mono">{r.id.slice(0, 18)}…</span>
                  </td>
                  ...
                  <td className="px-3 py-3.5">
                    ...
                    <span className="block text-[11px] text-slate-400 font-mono truncate max-w-[120px]">{r.reporter_id.slice(0, 8)}…</span>
                  </td>
                  ...
                  <td className="px-3 py-3.5 text-slate-500 whitespace-nowrap text-xs">
                    <span className="inline-flex items-center gap-1"><Calendar className="w-3 h-3 text-slate-300" />{formatDate(r.created_at)}</span>
                  </td>
                  <td className="px-3 py-3.5 text-xs max-w-[120px] truncate">
                    {r.assigned_to_name ? <span className="font-semibold text-[#0F172A]" title={r.assigned_to_name}>{r.assigned_to_name}</span> : <span className="text-slate-400">Unassigned</span>}
                  </td>
                  <td className="pr-5 pl-3 py-3.5">
                    <div className="flex items-center gap-1">
                      <button onClick={() => setSelected(r)} title="Investigate / view details" className="p-2 rounded-lg hover:bg-red-50 hover:text-[#E53935] text-slate-500 transition-colors">
                      ...
                      {r.status === 'open' && (
                        <button
                          ...
                          className="p-2 rounded-lg hover:bg-blue-50 hover:text-blue-700 text-slate-500 transition-colors disabled:opacity-40"
                        >
          ...
          <p className="text-center text-[11px] text-slate-400">Page {page} of {totalPages} — {total.toLocaleString('en-IN')} reports • {PAGE_SIZE}/page</p>
        </>
      )}
        ...
        {selected && (
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-[11px] text-slate-400">Created {formatDateTime(selected.created_at)}</span>
              {selected.resolved_at && <span className="text-[11px] text-slate-400">Resolved {formatDateTime(selected.resolved_at)}</span>}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="bg-slate-50 rounded-xl px-3 py-3 border border-slate-100">
                <p className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Reporter &amp; Target</p>
                <p className="text-xs mt-1"><span className="text-slate-400">Reporter:</span> <span className="font-semibold text-[#0F172A]">{selected.reporter_name}</span> <span className="font-mono text-[11px] text-slate-400">({selected.reporter_id.slice(0, 8)}…)</span></p>
                <p className="text-xs mt-1"><span className="text-slate-400">Target:</span> <span className="font-semibold">{getTargetLabel(selected)}</span></p>
                {selected.reported_user_id && <p className="text-[11px] text-slate-500 mt-1 font-mono">User {selected.reported_user_id}</p>}
                {selected.ad_id && <p className="text-[11px] text-slate-500 font-mono">Ad {selected.ad_id}</p>}
                {selected.message_id && <p className="text-[11px] text-slate-500 font-mono">Msg {selected.message_id}</p>}
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              {investigation?.loading ? (
                <div className="bg-slate-50 rounded-2xl border border-slate-100 p-8 flex items-center justify-center gap-2 text-slate-400">
                ...
                <div className="grid lg:grid-cols-2 gap-4">
                  ...
                  <div className="bg-white rounded-2xl border border-slate-100 p-4 space-y-3">
                    <h5 className="text-xs font-black uppercase tracking-wide text-slate-400 flex items-center gap-1.5"><ShoppingBag className="w-3.5 h-3.5" /> Reported ad</h5>
                    {investigation?.ad ? (
                      <div className="space-y-2 text-sm">
                        <p className="font-bold text-[#0F172A]">{investigation.ad.title}</p>
                        <p className="text-xs text-slate-500">{investigation.ad.category} · {investigation.ad.location} · <span className="capitalize">{investigation.ad.status}</span> · ₹{investigation.ad.price != null ? Number(investigation.ad.price).toLocaleString('en-IN') : '—'}</p>
                        ...
                        <p className="text-[11px] text-slate-400">Seller: <span className="font-semibold text-slate-600">{investigation.ad.sellerName}</span> · Created {formatDate(investigation.ad.created_at)}</p>
                      ...
                      <p className="text-xs text-slate-500">Ad {selected.ad_id.slice(0, 8)}… — details unavailable (deleted or no permission).</p>
                    ) : (
                      <p className="text-xs text-slate-400">No ad associated with this report.</p>
                    )}
                  </div>

                  {/* Reported user */}
                  <div className="bg-white rounded-2xl border border-slate-100 p-4 space-y-3">
                    <h5 className="text-xs font-black uppercase tracking-wide text-slate-400 flex items-center gap-1.5"><User className="w-3.5 h-3.5" /> Reported user</h5>
                    {investigation?.reportedUser ? (
                      <div className="space-y-2 text-sm">
                        <p className="font-bold text-[#0F172A]">{investigation.reportedUser.name}</p>
                        <p className="text-xs text-slate-500 truncate">{investigation.reportedUser.email}</p>
                        <div className="flex flex-wrap gap-1.5">
                          ...
                          {investigation.reportedUser.is_verified ? <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[11px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-100"><ShieldCheck className="w-3 h-3" /> Verified</span> : <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[11px] font-bold bg-slate-100 text-slate-500 border">Unverified</span>}
                        </div>
                      ...
                      <p className="text-xs text-slate-500">User {selected.reported_user_id.slice(0, 8)}… — profile unavailable.</p>
                    ) : (
                      <p className="text-xs text-slate-400">No user directly reported.</p>
                    )}
                    {investigation?.otherAds && investigation.otherAds.length > 0 && (
                      <div className="pt-2 border-t border-slate-100">
                        <p className="text-[11px] font-bold uppercase tracking-wide text-slate-400 flex items-center gap-1"><Layers className="w-3 h-3" /> Other ads by this user ({investigation.otherAds.length})</p>
                        <ul className="mt-2 space-y-1.5">
                          {investigation.otherAds.map((a) => (
                            <li key={a.id} className="flex items-center justify-between text-xs bg-slate-50 rounded-xl px-3 py-2 border border-slate-100">
                              ...
                              <span className="text-slate-500 shrink-0 ml-2">{a.status}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
                  ...
                  {/* Reported message */}
                  <div className="bg-white rounded-2xl border border-slate-100 p-4 space-y-3">
                    <h5 className="text-xs font-black uppercase tracking-wide text-slate-400 flex items-center gap-1.5"><MessageSquare className="w-3.5 h-3.5" /> Reported message</h5>
                    {investigation?.message ? (
                      <div className="space-y-2 text-sm">
                        ...
                        <p className="text-[11px] text-slate-400">From <span className="font-mono">{investigation.message.sender_id.slice(0, 8)}…</span> · Conversation {investigation.message.conversation_id.slice(0, 8)}… · {formatDateTime(investigation.message.created_at)}</p>
                      ...
                      <p className="text-xs text-slate-500">Message {selected.message_id.slice(0, 8)}… — content unavailable (RLS).</p>
                    ) : (
                      <p className="text-xs text-slate-400">No message associated.</p>
                    )}
                  </div>

                  {/* Related conversations */}
                  <div className="bg-white rounded-2xl border border-slate-100 p-4 space-y-3">
                    <h5 className="text-xs font-black uppercase tracking-wide text-slate-400 flex items-center gap-1.5"><MessageSquare className="w-3.5 h-3.5" /> Related conversations</h5>
                        ...
                        {investigation.relatedConversations.map((c) => (
                          <li key={c.id} className="flex items-center justify-between text-xs bg-slate-50 rounded-xl px-3 py-2.5 border border-slate-100">
                            <span className="min-w-0">
                              ...
                              <span className="block text-[11px] text-slate-400">{c.buyer_name} ↔ {c.seller_name}</span>
                            </span>
                            <span className="text-[11px] text-slate-400 shrink-0">{formatDate(c.updated_at)}</span>
                          </li>
                        ))}
                      </ul>
                    ) : (
                      <p className="text-xs text-slate-400">No related conversations found.</p>
                    )}
                  </div>

                  {/* Previous reports */}
                  <div className="bg-white rounded-2xl border border-slate-100 p-4 space-y-3">
                    <h5 className="text-xs font-black uppercase tracking-wide text-slate-400 flex items-center gap-1.5"><Flag className="w-3.5 h-3.5" /> Previous reports ({investigation?.previousReports.length ?? 0})</h5>
                        ...
                        {investigation.previousReports.map((pr) => (
                          <li key={pr.id} className="text-xs bg-slate-50 rounded-xl px-3 py-2 border border-slate-100">
                            <span className="flex items-center gap-1.5">
                              ...
                              <span className="text-slate-500">{formatDate(pr.created_at)}</span>
                            </span>
                            <span className="block mt-1 font-semibold">{pr.reason}</span>
                            <span className="block text-slate-500 truncate">{pr.description || '—'}</span>
                          </li>
                        ))}
                      </ul>
                    ) : (
                      <p className="text-xs text-slate-400">No prior reports for this target.</p>
                    )}
                  </div>

                  {/* Moderation history */}
                  <div className="bg-white rounded-2xl border border-slate-100 p-4 space-y-3">
                    <h5 className="text-xs font-black uppercase tracking-wide text-slate-400 flex items-center gap-1.5"><History className="w-3.5 h-3.5" /> Moderation history</h5>
                        ...
                        {investigation.moderationHistory.map((h) => (
                          <li key={h.id} className="flex items-start gap-2 text-xs bg-slate-50 rounded-xl px-3 py-2 border border-slate-100">
                            ...
                            <span className="min-w-0">
                              ...
                              <span className="block text-[11px] text-slate-400">{formatDateTime(h.created_at)}</span>
                              {h.metadata ? <span className="block text-[11px] text-slate-500 font-mono truncate">{JSON.stringify(h.metadata).slice(0, 120)}</span> : null}
                            </span>
                          </li>
                        ))}
                      </ul>
                    ) : (
                      <p className="text-xs text-slate-400">No audit entries for this report / target. Actions are written to admin_audit_logs when available.</p>
                    )}
                  </div>
                </div>
              )}
