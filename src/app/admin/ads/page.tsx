    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-700 mt-1">Moderate listings before they go live. <span className="font-black text-amber-600">{tabCounts['Pending'] ?? 0} awaiting review</span> • {ads.length} total</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-red-100 p-10 text-center">
          ...
          <p className="text-xs text-slate-700 mt-2">Check console for technical details. RLS: ensure admin role is set and is_admin() returns true.</p>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-14 text-center">
          ...
          <p className="text-xs text-slate-700 mt-1">Try another filter or search term.</p>
        </div>
      ) : (
        <>
                  ...
                  <tr key={ad.id} className="hover:bg-slate-50/60 transition-colors">
                    <td className="pl-5 pr-3 py-3.5">
                      <div className="flex items-center gap-3 min-w-0 max-w-[280px]">
                        {ad.image ? <img src={ad.image} alt="" className="w-12 h-9 rounded-lg object-cover shrink-0 bg-slate-100" /> : <span className="w-12 h-9 rounded-lg bg-slate-100 shrink-0 flex items-center justify-center text-[10px] text-slate-700">No img</span>}
                        <span className="min-w-0">
                          <span className="block font-bold truncate text-[#0F172A]" title={ad.title}>{ad.title}</span>
                          <span className="block text-[11px] text-slate-700">₹{ad.price.toLocaleString('en-IN')} • {ad.id.slice(0,8)} {ad.isFeatured && <span className="ml-1 inline-flex px-1.5 py-0.5 rounded bg-amber-50 border border-amber-200 text-amber-700 text-[9px] font-bold">⭐ Featured</span>}</span>
                        </span>
                      </div>
                    </td>
                    <td className="px-3 py-3.5 whitespace-nowrap">
                      <span className="block text-xs font-semibold text-[#0F172A]">{ad.seller}</span>
                      {ad.sellerEmail && <span className="block text-[11px] text-slate-700 truncate max-w-[140px]">{ad.sellerEmail}</span>}
                    </td>
                    ...
                    <td className="px-3 py-3.5 text-slate-700 whitespace-nowrap text-xs">{ad.date}</td>
                    <td className="pr-5 pl-3 py-3.5">
                      <div className="flex items-center gap-1 justify-end">
                        <button onClick={() => openDetail(ad)} title="View full details" className="p-2 rounded-lg hover:bg-red-50 hover:text-[#E53935] text-slate-700 transition-colors"><Eye className="w-4 h-4" /></button>
                        ...
                        {(ad.status as string) === 'approved' && <button onClick={() => setSuspendTarget(ad)} title="Suspend" className="p-2 rounded-lg hover:bg-orange-50 hover:text-orange-700 text-slate-700 transition-colors"><Ban className="w-4 h-4" /></button>}
          ...
          <p className="text-center text-[11px] text-slate-700">Page {page} of {totalPages} — {filteredVisible.length} filtered • {ads.length} total</p>
        </>
      )}
      ...
      <Modal open={viewing !== null} onClose={() => { setViewing(null); setViewingDetail(null); }} title="Moderation Review" size="lg">
        {viewing && (
          <div className="space-y-5">
            {detailLoading ? <p className="text-sm text-slate-700 py-6 text-center animate-pulse">Loading full advertisement...</p> : viewingDetail ? (
              <>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  ...
                  {(viewingDetail.imagesFull?.length ?? 0) === 0 && <div className="col-span-full py-8 text-center text-xs text-slate-700 bg-slate-50 rounded-xl border border-slate-100">No images</div>}
                </div>
                <div className="flex gap-4">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-700 mt-1">By {viewingDetail.seller} {viewingDetail.sellerEmail && <span className="text-slate-600">· {viewingDetail.sellerEmail}</span>} · {viewingDetail.location} · {viewingDetail.date}</p>
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-100"><span className="block text-[11px] font-bold text-slate-700 uppercase">Category</span><span className="font-semibold text-[#0F172A]">{viewingDetail.category}{viewingDetail.subcategory ? ` › ${viewingDetail.subcategory}` : ''}</span></div>
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-100"><span className="block text-[11px] font-bold text-slate-700 uppercase">Location</span><span className="font-semibold text-[#0F172A]">{viewingDetail.location}</span></div>
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-100"><span className="block text-[11px] font-bold text-slate-700 uppercase">Seller</span><span className="font-semibold text-[#0F172A]">{viewingDetail.seller}</span><span className="block text-[11px] text-slate-700">{viewingDetail.sellerEmail} {viewingDetail.sellerPhone && `· ${viewingDetail.sellerPhone}`}</span></div>
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-100"><span className="block text-[11px] font-bold text-slate-700 uppercase">Views / Featured</span><span className="font-semibold text-[#0F172A]">{viewingDetail.viewsCount ?? 0} views {viewingDetail.isFeatured ? '• Featured' : ''}</span></div>
                </div>
                <div>
                ...
                {viewingDetail.attributes && Object.keys(viewingDetail.attributes).length>0 && (
                  <div>
                    <p className="text-xs font-bold uppercase tracking-wide text-slate-700 mb-1.5">Attributes</p>
                    <div className="flex flex-wrap gap-1.5">
                      {Object.entries(viewingDetail.attributes).map(([k,v])=> <span key={k} className="px-2.5 py-1 rounded-full bg-white border border-slate-200 text-xs"><span className="font-bold text-[#0F172A]">{k}:</span> <span className="text-slate-700">{String(v)}</span></span>)}
                    </div>
                  </div>
                )}
                {viewingDetail.rejectionReason && <div className="p-3 rounded-xl bg-red-50 border border-red-100 text-xs"><span className="font-bold text-[#D32F2F]">Rejection reason:</span> <span className="text-slate-700">{viewingDetail.rejectionReason}</span></div>}
                <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <p className="text-sm text-slate-700">Failed to load details.</p>
            )}
          </div>
        )}
      </Modal>
      ...
      <Modal open={historyAd !== null} onClose={() => setHistoryAd(null)} title="Moderation History" size="md">
        {historyLoading ? <p className="text-sm text-slate-700 py-6 text-center">Loading history...</p> : historyItems.length === 0 ? <p className="text-sm text-slate-700 py-6 text-center">No moderation history for this ad.</p> : (
          <ul className="space-y-3">
            {historyItems.map((h,i) => (
              <li key={i} className="flex gap-3 text-sm">
                <span className="w-2 h-2 rounded-full bg-[#E53935] mt-2 shrink-0" />
                <span><span className="font-bold">{h.action}</span> <span className="text-slate-700">· {new Date(h.created_at).toLocaleString('en-IN')}</span><span className="block text-xs text-slate-700 mt-1">{h.metadata?.reason || JSON.stringify(h.metadata || {}).slice(0,120)}</span></span>
              </li>
            ))}
          </ul>
        )}
      ...
      <Modal open={requestTarget !== null} onClose={() => setRequestTarget(null)} title="Request Changes" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-slate-700">Seller will be notified and can edit and resubmit. Status becomes <span className="font-bold">changes_requested</span>.</p>
      ...
      <Modal open={suspendTarget !== null} onClose={() => setSuspendTarget(null)} title="Suspend Advertisement" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-slate-700">Suspended ads are hidden from public until unsuspended.</p>
      ...
      <Modal open={rejectTarget !== null} onClose={() => setRejectTarget(null)} title="Reject Advertisement" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-slate-700">The seller will be notified with this reason and can edit & resubmit.</p>
