    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Manage banner campaigns across the platform.</p>
        </div>
        <button
      ...
      <DataTable headers={['Advertiser', 'Banner', 'Placement', 'Impressions', 'Clicks', 'CTR', 'Price', 'Status']}>
        {visible.map((c) => (
          <tr key={c.id} className="hover:bg-slate-50/60 transition-colors">
            ...
            <td className="px-3 py-3.5 text-slate-500 max-w-[220px] truncate">{c.bannerText}</td>
            ...
            <td className="pr-5 pl-3 py-3.5">
              <span className={`inline-flex px-2.5 py-1 rounded-full text-[11px] font-bold ${
                c.status === 'Active' ? 'bg-emerald-50 text-emerald-700' : c.status === 'Scheduled' ? 'bg-sky-50 text-sky-700' : 'bg-slate-100 text-slate-500'
              }`}>{c.status}</span>
            </td>
          </tr>
        ))}
